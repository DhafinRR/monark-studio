...
export default function InvoicePreview({
  ...
  const addonTotal = addonItems.reduce((sum, i) => sum + Number(i.price || 0), 0)
  const floorPrice = standardItems.length > 0 ? totalAmount - addonTotal : (addonItems.length > 0 ? 0 : totalAmount)
    ...
    <>
      <div className="bg-white min-h-[1100px] flex flex-col print:min-h-0 print:h-auto print:bg-white selection:bg-gray-100 border border-gray-200 rounded-lg shadow-sm overflow-hidden" data-invoice>
          ...
          <div className="grid grid-cols-2 gap-16 print:gap-6">
            <div className="space-y-3 print:space-y-0.5">
              ...
              <div className="space-y-1 print:space-y-0">
                <p className="text-xl font-semibold text-gray-900 print:text-sm tracking-tight">{customerName || 'Client Name'}</p>
                <p className="text-sm text-gray-600 print:text-[10px]">WA: {customerPhone || '-'}</p>
                {customerEmail && <p className="text-sm text-gray-600 print:text-[10px]">{customerEmail}</p>}
              </div>
            </div>
            <div className="space-y-3 print:space-y-0.5">
              <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider print:text-[9px]">Project</h3>
              <p className="text-lg font-medium text-gray-900 print:text-xs italic">{projectTitle || pricingPackageName || 'Development'}</p>
              {pricingPackageName && (
                <p className="text-xs text-amber-600 font-medium print:text-[9px]">Package: {pricingPackageName}</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="px-16 py-12 flex-1 print:px-6 print:py-3 space-y-10 print:space-y-3">
          ...
          {/* If no classification items, fallback list */}
          {standardItems.length === 0 && addonItems.length === 0 && items.length > 0 && (
            <div>
              <div className="grid grid-cols-12 gap-4 pb-3 mb-4 border-b-2 border-gray-900 print:mb-1 print:pb-1">
                <div className="col-span-1 text-xs font-bold text-gray-900 uppercase tracking-widest print:text-[9px]">No</div>
                <div className="col-span-8 text-xs font-bold text-gray-900 uppercase tracking-widest print:text-[9px]">Item Description</div>
                <div className="col-span-3 text-xs font-bold text-gray-900 uppercase tracking-widest text-right print:text-[9px]">Amount (IDR)</div>
              </div>
              <div className="space-y-3 print:space-y-0.5">
                {items.map((item, idx) => (
          ...
          <div className="flex justify-end">
            <div className="w-96 space-y-4 print:w-64 print:space-y-0.5">
              ...
              <div className="flex justify-between py-6 bg-gray-900 px-8 -mx-8 print:py-2 print:px-4 print:-mx-4 shadow-xl print:shadow-none">
                ...
                <span className="text-2xl font-black text-white print:text-base italic">Rp {totalAmount.toLocaleString('id-ID')}</span>
              </div>
            </div>
          </div>
        </div>
