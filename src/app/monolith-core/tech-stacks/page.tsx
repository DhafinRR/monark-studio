...
import { Plus, Search, ChevronUp, ChevronDown, ArrowUpDown, Code, Trash2, Edit } from 'lucide-react'
...
export default function TechStacksPage() {
    ...
    fetch('/api/monolith-core/tech-stacks')
      ...
      .then(data => {
        if (Array.isArray(data)) {
          ...
          console.error("Failed to load tech stacks:", data)
          setTechStacks([])
        }
        setLoading(false)
      })
  ...
  const processedData = useMemo(() => {
    let result = [...techStacks]

    // 1. Filtering berdasarkan Nama
    if (searchTerm) {
    ...
    // 2. Sorting (Hanya untuk Nama dan Warna)
    if (sortConfig) {
      result.sort((a, b) => {
    ...
    return result
  }, [techStacks, searchTerm, sortConfig])
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                <tr><td colSpan={5} className="px-6 py-20 text-center text-gray-500 animate-pulse font-medium">Memuat data teknologi...</td></tr>
              ) : processedData.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-20 text-center">
                    <div className="flex flex-col items-center gap-2">
                       <Code className="w-10 h-10 text-gray-200" />
                       <span className="text-gray-500 font-medium">Belum ada teknologi yang ditambahkan.</span>
                    </div>
                  </td>
                </tr>
              ) : (
