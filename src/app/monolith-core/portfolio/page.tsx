...
import { Plus, Search, ChevronUp, ChevronDown, ArrowUpDown, Briefcase, Trash2, Edit } from 'lucide-react'
...
export default function PortfolioPage() {
  ...
  const fetchData = () => {
    setLoading(true)
    fetch('/api/monolith-core/portfolio')
      .then(res => res.json())
      .then(data => {
  ...
  const processedData = useMemo(() => {
    ...
    // Filter by title, description, and stack names
    if (searchTerm) {
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                <tr><td colSpan={6} className="px-6 py-20 text-center text-gray-500 animate-pulse font-medium">Memuat data portfolio...</td></tr>
              ) : processedData.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-20 text-center">
                    <div className="flex flex-col items-center gap-2">
                      <Briefcase className="w-10 h-10 text-gray-200" />
                      <span className="text-gray-500 font-medium">Belum ada portfolio yang ditambahkan.</span>
                    </div>
                  </td>
                </tr>
              ) : (
